/* ================================================================
   ...
   Meta-driven: liest meta.json relativ zur aktuellen Page.
   ================================================================ */

let META = {
  // Default-Fallback falls meta.json nicht ladbar
  slug: 'unknown',
  name: 'Casting',
  storageKey: 'wexplore-casting-unknown-v1',
  web3formsKey: null,
  mailSubject: 'Casting-Auswahl',
  mailTitleLine: 'CASTING AUSWAHL',
};
...
async function loadMeta() {
  try {
    ...
    console.warn('meta.json konnte nicht geladen werden, Fallback aktiv', e);
  }
}
...
function saveStorage() {
  try {
    localStorage.setItem(META.storageKey, JSON.stringify({
      votes: state.votes,
      notes: state.notes,
    }));
  } catch (e) {
...
$sendForm.addEventListener('submit', async (e) => {
  ...
  try {
    const res = await fetch('https://api.web3forms.com/submit', {
      ...
      body: JSON.stringify({
        access_key: META.web3formsKey,
        subject: `${META.mailSubject} - von ${name}`,
        from_name: name,
        message: text,
        botcheck: ''
      })
